...
export const registerUserHandler = async (req: Request<{}, {}, CreateUserInput>, res: Response, next: NextFunction) => {
	try {
		...
		/**
		 * Send Verification Email (will use hosting url in production set in env variable)
		 * in dev use angular app url (using api route now to test)
		 */
...
export const verifyEmailHandler = async (req: Request<VerifyEmailInput>, res: Response, next: NextFunction) => {
	try {
		// Hash verification code to compare with the one in the database made on register
		// We sent the unhashed verification code in the email
		const verificationCode = crypto.createHash('sha256').update(req.params.verificationCode).digest('hex');
...
export const refreshAccessTokenHandler = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		const decoded = verifyJwt<{sub: string}>(refresh_token, 'jwtAccessToken');
		...
		const access_token = signJwt({sub: user.id}, 'jwtAccessToken', {
